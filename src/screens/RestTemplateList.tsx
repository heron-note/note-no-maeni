...
import { REST_TEMPLATE_MAX, type RestTemplate } from '../types'

export function RestTemplateList() {
  const goTo = useAppStore(s => s.goTo)
  const setEditingId = useAppStore(s => s.setEditingRestTemplateId)
  const { closing, handleBack } = useSlideBack(() => goTo('home'))

  const [templates, setTemplates] = useState<RestTemplate[]>(() => storage.loadRestTemplates())
  const [toast, setToast] = useState<string | null>(null)
  const [confirmDeleteId, setConfirmDeleteId] = useState<string | null>(null)

  const handleAdd = () => {
    setEditingId(null)
    goTo('rest-template-editor')
  }
    ...
    <div className={`screen-scroll${closing ? ' screen-slide-out' : ''}`}>
      <div className="subscreen-header" style={{flexDirection:'column', alignItems:'flex-start', gap:4}}>
        <div className="subscreen-title-row">
          ...
          <h2 className="subscreen-title">休むテンプレート管理</h2>
        </div>
        <p className="hint" style={{margin:0}}>「休む」時にコピーして使えるテンプレート（最大{REST_TEMPLATE_MAX}件）</p>
      </div>
      ...
      <button
        ...
        onClick={handleAdd}
        disabled={templates.length >= REST_TEMPLATE_MAX}
      >
        {templates.length >= REST_TEMPLATE_MAX
          ? `上限${REST_TEMPLATE_MAX}件に達しました`
          : '+ 新しいテンプレートを追加'}
      </button>
